...
namespace gpstk
{
                                            ...
                                            const satVectorMap& states )
    {
             ...
             ++it )
        {
            ...
            Matrix<double> da_dp(3,numSRPC,0.0);
            ...
            Matrix<double> dr_dp0(3,numSRPC,0.0), dv_dp0(3,numSRPC,0.0);
            ...
            for(int i=0; i<3; ++i)
            {
                ...
                {
                    dr_dp0(i,j) = state(42+numSRPC*(i+0)+j);
                    dv_dp0(i,j) = state(42+numSRPC*(i+3)+j);
                }
            }
            ...
            for(int i=0; i<3; ++i)
            {
                ...
                for(int j=0; j<numSRPC; ++j)
                {
                    dState(42+numSRPC*(0+i)+j) = dv_dp0(i,j);
                    dState(42+numSRPC*(3+i)+j) = da_dp0(i,j);
                }
            }

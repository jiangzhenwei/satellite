...
namespace gpstk
{
     ...
     * @param orbits    orbits
     */
    void ECOMModel::Compute( const CommonTime& tt,
                             const satVectorMap& orbits )
    {
             ...
             ++it )
        {
            ...
            D0 = coeff(0); DC = coeff(1); DS = coeff(2);
            Y0 = coeff(3); YC = coeff(4); YS = coeff(5);
            B0 = coeff(6); BC = coeff(7); BS = coeff(8);
...
//            sp_unit = cross(sa_unit, nop_unit);
            ...
            /// acceleration
            Du = D0 + DC*cosu_sat + DS*sinu_sat;
            Yu = Y0 + YC*cosu_sat + YS*sinu_sat;
            ...
            for(int i=0; i<3; ++i)
            {
                ...
                da_dSRP(i,1) = d_unit(i) * cosu_sat;    // da / dDC
                da_dSRP(i,2) = d_unit(i) * sinu_sat;    // da / dDS
                da_dSRP(i,3) = y_unit(i);               // da / dY0
                da_dSRP(i,4) = y_unit(i) * cosu_sat;    // da / dYC
                da_dSRP(i,5) = y_unit(i) * sinu_sat;    // da / dYS
                da_dSRP(i,6) = b_unit(i);               // da / dB0
                da_dSRP(i,7) = b_unit(i) * cosu_sat;    // da / dBc
                da_dSRP(i,8) = b_unit(i) * sinu_sat;    // da / dBs
            }

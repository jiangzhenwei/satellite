#pragma ident "$Id XY. CAO 2-17-11-14 $"
...
namespace gpstk
{
      ...
      /* Return the effect of ocean tides loading (meters) at the given
       ...
       */
    Triple OceanLoading::getOceanLoading( const CommonTime& time,
                                          const string& name )
        throw(InvalidRequest)
    {
        ...
        // Change the time system from GPST to UTC(UT1)
        CommonTime utc( pRefSys->GPS2UTC(time) );

        arguments = getArg(utc);
    ...
    Vector<double> OceanLoading::getArg(const CommonTime& time)
    {
        ...
        // This is original code which use four digital year as input parameter. However,
        // according to IERS2010(ARG2.F), two digital year is used and the results are different.
        // After test, the four digital may be correct.
        double d( static_cast<YDSTime>(time).doy + 365.0*(year-1975.0)
                                                 + floor((year-1973.0)/4.0) );
        ...
        double S0( (((1.9e-6*t - 0.001133)*t + 481267.88314137)*t
                                                + 270.434358) * DEG_TO_RAD );

        // Mean longitude of lunar perigee at beginning of day
        double P0( (((-1.2e-5*t - 0.010325)*t + 4069.0340329577)*t
                                                + 334.329653) * DEG_TO_RAD );

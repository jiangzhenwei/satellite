...
namespace gpstk
{
                                    ...
                                    double& weight)
   {
      ...
      if ( (flag1!=0.0) || (flag2!=0.0) )
      {
         ...
         SmoothingData[sat].previousVar = varPC; // Set the variance of PC as 1.0m;
      ...
      smoothedCode = (code + (wSize-1.0)*( prevCode + (phase-prevPhase) ) ) / wSize;
         ...
         // The variance of PC is 1.0m;
      varSmCode = (varPC + (wSize-1.0)*(wSize-1.0)*prevVar ) /(wSize*wSize);

         // Weight for smoothed code is var(PC)/var(smoothedCode)
      weight = varPC/varSmCode;

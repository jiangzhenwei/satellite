...
//   std::istream& operator<<(std::istream& s, SatID& p);
    ...
    public:
        ...
        static std::string convertSatelliteSystemToString(SatelliteSystem s)
        {
            switch(s)
            {
                case systemGPS:         return "G";             break;
                case systemGLONASS:     return "R";             break;
                case systemGalileo:     return "E";             break;
                case systemSBAS:        return "S";             break;
                case systemQZSS:        return "J";             break;
                case systemBDS:         return "C";             break;
                case systemIRNSS:       return "I";             break;
        ...
        {
            s << convertSatelliteSystemToString(system) << " " << std::setw(2)  << id;
        }
        ...
        ///     plus GPS id's are less than or equal to MAX_PRN_GPS,
        ///          GAL id's are less than or equal to MAX_PRN_GAL,
        ///          GLO id's are less than or equal to MAX_PRN_GLO,
        ///          BDS id's are less than or equal to MAX_PRN_BDS.
        ...
        bool isValid() const
        {
            switch(system)
            {
                ...
                case systemGLONASS: return (id > 0 && id <= MAX_PRN_GLO);
                case systemGalileo: return (id > 0 && id <= MAX_PRN_GAL);
                case systemBDS:     return (id > 0 && id <= MAX_PRN_BDS);
                case systemLEO:     return (id > 200);

...
namespace gpstk
{
    ...
    Rinex3ObsHeader::Rinex3ObsHeader() : PisY(false)
    { clear(); }
...
//            map<string, map<TypeID, map<RinexSatID,double> > >::const_iterator it;
...
//                map<TypeID, map<RinexSatID,double> >::const_iterator jt(it->second.begin());
                if(jt == it->second.end())
                {
                    line  = sys;
...
//                        TypeID obsid(jt->first);
    ...
    void Rinex3ObsHeader::prepareVer2Write(void)
    {
        ...
        for (mit = mapObsTypes.begin(); mit != mapObsTypes.end(); mit++)
        {
            ...
            {
                FFStreamError er("Invalid system char string in header.mapObsTypes: "
                                    +sysString);
                GPSTK_THROW(er);
            }
                    ...
                    else
                    {
                        ...
                        if(R2DisambiguityMap.find(sysString + R2ot) == R2DisambiguityMap.end())
                            R2DisambiguityMap.insert(
                                std::pair<string,string>(sysString + R2ot,mapR2toR3ObsID[R2ot].asString())
                                                    );
                        else

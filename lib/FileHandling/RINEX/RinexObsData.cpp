        ...
        throw(std::exception, FFStreamError, gpstk::StringUtils::StringException)
    {
        ...
        *this=rod;
        ...
        if ((epochFlag < 0) || (epochFlag > 6))
        {
         ...
         // If there is an epoch time, parse it and load it in
         // the member "time".
        ...
        if(debug) cout << CivilTime(time) << endl;

        numSvs = asInt(line.substr(29,3));

        if(debug) cout << numSvs << endl;
        ...
        if(debug) cout << clockOffset << endl;
            ...
            for (isv=0; isv < numSvs; isv++)
            {
                ...
                for (ndx=0, line_ndx=0; ndx < numObs; ndx++, line_ndx++)
                {
                    ...
                    if(debug)
                    {
                        ...
                        cout << sat << ' '
                             << obs_type.type << ' '
                             << obs[sat][obs_type].data << ' '
                             << obs[sat][obs_type].lli << ' '
                             << obs[sat][obs_type].ssi << endl;
                    }
                }
            }
        }

...
namespace gpstk
{
    ...
    const string RinexObsHeader::antennaDeltaHENString =    "ANTENNA: DELTA H/E/N";
    const string RinexObsHeader::antennaDeltaXYZString =    "ANTENNA: DELTA X/Y/Z";
    const string RinexObsHeader::antennaBSightXYZString =   "ANTENNA: B.SIGHT XYZ";
    ...
    const RinexObsType RinexObsHeader::LA("LA", "LA Carrier Phase", "LA cycles", 0);
    const RinexObsType RinexObsHeader::DA("DA", "Doppler Frequency LA", "Hz", 0);
    const RinexObsType RinexObsHeader::SA("SA", "Signal-to-Noise LA", "dB-Hz", 0);
    const RinexObsType RinexObsHeader::CH("CH", "Reciver channel number", "dimensionless", 0);
    ...
    RinexObsType sot[33] =
    {
        ...
        RinexObsHeader::LA, RinexObsHeader::DA, RinexObsHeader::SA, RinexObsHeader::CH
    };

    // Warning: the size of the above sot array needs to be put
    // in this initializer.
    const std::vector<RinexObsType> RinexObsHeader::StandardRinexObsTypes(sot,sot+33);
    ...
    int RinexObsHeader::NumberHeaderRecordsToBeWritten(void) const throw()
    {
        ...
        if(valid & RinexObsHeader::markerTypeValid) n++;        // rinex 2.20
        ...
        if(valid & RinexObsHeader::antennaDeltaHENValid) n++;
        if(valid & RinexObsHeader::antennaDeltaXYZValid) n++;   // rinex 2.20
        if(valid & RinexObsHeader::antennaBSightXYZValid) n++;  // rinex 2.20
        ...
        throw(FFStreamError, StringException)
    {
        ...
        if (valid & markerTypeValid)        // rinex 2.20
        {
        ...
        if (valid & antennaDeltaHENValid)
        {
            ...
            line += string(18, ' ');
            line += antennaDeltaHENString;
            strm << line << endl;
            strm.lineNumber++;
        }
        if (valid & antennaDeltaXYZValid)
        {
            ...
            line += string(18, ' ');
            line += antennaDeltaXYZString;
            strm << line << endl;
            strm.lineNumber++;
        }
        if (valid & antennaBSightXYZValid)
        {
        ...
        throw(FFStreamError)
    {
        ...
        if (label == versionString)
        {
            ...
            if(debug) cout << version << ' ' << fileType << ' ' << system << endl;
            valid |= versionValid;
        }
        else if (label == runByString )
        {
            ...
            if(debug) cout << fileProgram << ' ' << fileAgency << ' ' << date << endl;
            valid |= runByValid;
        }
        ...
        else if (label == observerString)
        {
            ...
            if(debug) cout << observer << ' ' << agency << endl;
            valid |= observerValid;
        }
        else if (label == receiverString)
        {
            ...
            if(debug) cout << recNo << ' ' << recType << ' ' << recVers << endl;
            valid |= receiverValid;
        }
        else if (label ==antennaTypeString)
        {
            ...
            if(debug) cout << antNo << ' ' << antType << endl;
        ...
        else if (label == antennaDeltaHENString)
        {
            ...
            valid |= antennaDeltaHENValid;
        }
        else if (label == antennaDeltaXYZString)
        {
            ...
            if(debug) cout << antennaDeltaXYZ << endl;
            valid |= antennaDeltaXYZValid;
        }
        else if (label == antennaBSightXYZString)
        {
            ...
            if(debug) cout << antennaBSightXYZ << endl;
            valid |= antennaBSightXYZValid;
        }
        else if (label == waveFactString)
        {
            ...
            if (! (valid & waveFactValid))
            {
                ...
                if(debug) cout << wavelengthFactor[0] << ' ' << wavelengthFactor[1] << endl;
                valid |= waveFactValid;
            }
        ...
        else if (label == numObsString)
        {
            ...
            {
                cout << numObs << endl;
                for(vector<RinexObsType>::iterator it = obsTypeList.begin();
                    it != obsTypeList.end();
                    ++it)
                {
                    cout << (*it) << endl;
        ...
        else if (label == firstTimeString)
        {
            ...
            if(debug) cout << CivilTime(firstObs) << ' ' << firstSystem << endl;
        ...
        else if (label == lastTimeString)
        {
            ...
            if(debug) cout << CivilTime(lastObs) << ' ' << lastSystem << endl;
            valid |= lastTimeValid;
        }
        else if (label == receiverOffsetString)
        {
            receiverOffset = asInt(line.substr(0,6));
            if(debug) cout << receiverOffset << endl;
    ...
    RinexObsType RinexObsHeader::convertObsType(const string& oneObs)
        throw(FFStreamError)
    {
    ...
    string RinexObsHeader::convertObsType(const RinexObsType& oneObs)
        throw(FFStreamError)
    {
        return oneObs.type;
    }
    ...
    void RinexObsHeader::dump(ostream& s) const
    {
        ...
        s << "Antenna offset (ENU,m) : " << setprecision(4) << antennaDeltaHEN << ".\n";
        ...
        if(!(valid & antennaDeltaHENValid)) s << " Antenna Delta HEN is NOT valid\n";
        if(!(valid & antennaDeltaXYZValid)) s << " Antenna Delta XYZ is NOT valid\n";
        if(!(valid & antennaBSightXYZValid)) s << " Antenna B.Sight XYZ is NOT valid\n";
